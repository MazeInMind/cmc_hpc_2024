...
const int N = 40; // Размер глобальной сетки
...
// Функция решения уравнения Пуассона на локальной сетке
    ...
    for (int i = 1; i <= local_Nx; ++i) {
        for (int j = 1; j <= local_Ny; ++j) {
            ...
            if (insideEllipse(x, y)) {
                ...
                u_new[current] = 0.25 * (u_old[up] + u_old[down] +
                                         u_old[left] + u_old[right] +
                                         hx * hy * f(x, y));
                double diff = std::fabs(u_new[current] - u_old[current]);
                if (diff > tolerance) {
                ...
                int current = idx(i, j, local_Ny);
                u_new[current] = 0.0; // Граничное условие Дирихле
            }
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    // Получение координат процесса в 2D топологии
    int coords[2];
    MPI_Cart_coords(cart_comm, rank, 2, coords);
    int proc_row = coords[0]; // Координата процесса по y
    int proc_col = coords[1]; // Координата процесса по x
 
    // Проверка делимости сетки
    if (N % Px != 0 || N % Py != 0) {
        if (rank == 0) {
    ...
    MPI_Cart_shift(cart_comm, 0, 1, &north, &south); // По y
    MPI_Cart_shift(cart_comm, 1, 1, &west, &east);   // По x
        ...
        for (int proc = 0; proc < size; ++proc) {
            ...
            for (int i = 0; i < local_Nx; ++i) {
                for (int j = 0; j < local_Ny; ++j) {
                    int global_i = proc_row * local_Nx + i;
                    int global_j = proc_col * local_Ny + j;
